...
Server side routing will re-load the full page on a data request and return the data to the user. All of this is handled on the server.
Client side routing uses JS and is a much more efficient way to request data from a servers. The page will no longer refresh but the data will be retrieved from the server and returned to the browser without refreshing the page.
